...
const CourseGrid = ({ courses, onCourseSelect }) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
      {courses.map(course => (
        <div key={course._id || course.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6 hover:shadow-md transition-shadow">
          <div className="text-center mb-4">
            <div className="text-3xl md:text-4xl mb-2">{course.image}</div>
            <h3 className="text-base md:text-lg font-semibold text-gray-900 line-clamp-2">{course.title}</h3>
            <p className="text-gray-600 text-xs md:text-sm mt-1">{course.instructorName || course.instructor}</p>
          </div>
          ...
          <div className="space-y-2 md:space-y-3 text-xs md:text-sm text-gray-600">
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center space-x-1">
                ...
                <span>({(course.enrolledStudents || course.students || 0).toLocaleString()})</span>
              </div>
            </div>
          </div>
